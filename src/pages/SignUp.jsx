...
const SignUp = () => {
    ...
    const [showModalRegisterFaceID,setShowModalRegisterFaceID] = useState(false);
    const [messageApi, contextHolder] = message.useMessage();
    const [infoUser,setInfoUser] = useState(null)
    const handleOpenModalFaceID = ()=> setShowModalRegisterFaceID(true);
    const handleCloseModalFaceIO = ()=> setShowModalRegisterFaceID(false);
    ...
    const handleSubmit = (e) => {
        ...
        const sendUser = async () => {
            const response = await GetData.signUp({email,username,password})
                console.log(response);
                setInfoUser(response?.user)
                
                if(response.success) {
                    message.success(response.message)
                    handleOpenModalFaceID()
    ...
    const ModalRegisterFaceID = ()=> (
        
        <Modal
            open={showModalRegisterFaceID}
            okText="Đồng ý"
            cancelText="Huỷ"
            onCancel={handleCloseModalFaceIO}
            onOk={faceRegistration}
        >
            {contextHolder}
            <h3>
                Bạn có chắc chắn muốn đăng ký đăng nhập bằng nhận diện khuôn mặt?
            </h3>
        </Modal>
    )
  return (
        ...
        <div className="appRight">
            <form action='' className='infoForm' type='submit' onSubmit={handleSubmit}>
                <h3>Sign Up</h3>
                    ...
                    <div>
                        <input type="text" className="infoInput" placeholder='Name' name='name' onChange={handleChange} value={data.name}/>
                        <input type="email" className="infoInput" placeholder='Email' name='email' onChange={handleChange} value={data.email}/>
                    </div>
                ...
                <div style={{display: 'flex', flexDirection: 'column'}} >
                    <span className='faceid' style={{fontSize: 14, cursor: "pointer"}} onClick={() => {resetForm();navigate('/login');window.location.reload()}}>
